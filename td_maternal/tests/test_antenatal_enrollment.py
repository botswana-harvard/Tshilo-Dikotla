...
class TestAntenatalEnrollment(MotherMixin, TestCase):
    ...
    def test_gestation_wks_lmp_below_16(self):
        ...
        options = {'subject_identifier': self.maternal_identifier,
                   'last_period_date': (self.get_utcnow() - relativedelta(weeks=10)).date()}
        antenatal_enrollment = self.make_antenatal_enrollment(POS, **options)
    ...
    def test_gestation_wks_lmp_above_36(self):
        ...
        options = dict(
            ...
            last_period_date=(self.get_utcnow() - relativedelta(weeks=37)).date())
        antenatal_enrollment = self.make_antenatal_enrollment(POS, **options)
        self.assertFalse(antenatal_enrollment.is_eligible)
    ...
    def test_mother_tested_POS_at_32weeks_with_evidence(self):
        ...
        options = dict(
            ...
            week32_test=YES,
            week32_test_date=(self.get_utcnow() + relativedelta(weeks=5)).date(),
            week32_result=POS,
            evidence_32wk_hiv_status=YES,
            will_get_arvs=YES,
            last_period_date=(self.get_utcnow() - relativedelta(weeks=34)).date())
        antenatal_enrollment = self.make_antenatal_enrollment(**options)
        self.assertTrue(antenatal_enrollment.is_eligible)
    ...
    def test_mother_tested_POS_32weeks_with_NO_evidence(self):
        ...
        options = dict(
            ...
            evidence_hiv_status=None,
            week32_test=YES,
            week32_test_date=(self.get_utcnow() + relativedelta(weeks=1)).date(),
            week32_result=POS,
            evidence_32wk_hiv_status=NO,
            will_get_arvs=YES,
            rapid_test_done=YES,
            rapid_test_date=(self.get_utcnow() - relativedelta(days=1)).date(),
    ...
    def test_mother_untested_at_32weeks_undergoes_rapid(self):
        ...
        options = dict(
            ...
            will_get_arvs=NOT_APPLICABLE,
            rapid_test_done=YES,
            rapid_test_date=self.get_utcnow().date(),
            rapid_test_result=NEG,
            last_period_date=(self.get_utcnow() - relativedelta(weeks=35)).date())
        antenatal_enrollment = self.make_antenatal_enrollment(**options)
        self.assertTrue(antenatal_enrollment.is_eligible)
