...
class TestMaternalUltrasound(PosMotherMixin, TestCase):
    ...
    def test_pass_eligibility_on_singleton_pregnancy(self):
        ...
        antenatal_enrollment = AntenatalEnrollment.objects.get(subject_identifier=self.maternal_identifier)
    ...
    def test_ineligible_adds_offstudy(self):
        ...
        self.assertFalse(antenatal_enrollment.is_eligible)
        try:
            MaternalOffstudy.objects.get(subject_identifier=self.maternal_identifier)
    ...
    def test_ga_by_lmp(self):
        ...
        date."""
        maternal_visit = self.add_maternal_visit('1000M')
        maternal_ultrasound = self.make_ultrasound(maternal_visit)
        antenatal_enrollment = AntenatalEnrollment.objects.get(subject_identifier=self.maternal_identifier)
        lmp = Lmp(
...
#         self.assertEqual(antenatal_enrollment.enrollment_ga_method, ULTRASOUND)
    ...
    def test_no_lmp_with_ultrasound_gaconfirmed_eligible(self):
        ...
        maternal_ultrasound = self.make_ultrasound(
            maternal_visit,
            ga_by_ultrasound_days=4)
        self.assertIsNone(maternal_ultrasound.ga_by_lmp)
        self.assertEqual(maternal_ultrasound.ga_by_ultrasound_wks, 20)
        antenatal_enrollment = AntenatalEnrollment.objects.get(subject_identifier=self.maternal_identifier)
        self.assertTrue(antenatal_enrollment.is_eligible)
